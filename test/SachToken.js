...
it('handles delegated token transfers', function() {
  return SachToken.deployed().then(function(instance){
    ...
    return tokenInstance.transferFrom.call(fromAccount, toAccount, 10, {from: spendingAcount});
  }).then(function(success){
    assert.equal(success, true);
    return tokenInstance.transferFrom(fromAccount, toAccount, 10, {from: spendingAcount});
  }).then(function(receipt){
    assert.equal(receipt.logs.length,1,'triggers one event');
    assert.equal(receipt.logs[0].event, 'Transfer' ,'should be the "Trasfer" event');
    assert.equal(receipt.logs[0].args._from, fromAccount,'logs the account the tokens are transferrred from');
    assert.equal(receipt.logs[0].args._to, toAccount,'logs the account the tokens are transferrred to');
    ...
    return tokenInstance.allowance(fromAccount, spendingAcount);
  }).then(function(allowance) {
